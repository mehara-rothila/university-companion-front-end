  ...
  useEffect(() => {
    ...
    if (errorParam) {
      if (errorParam === 'session_expired') {
        setError(
          t('auth.login.sessionExpired') || 'Your session has expired. Please log in again.'
        );
      } else {
        setError(errorParam);
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result === true) {
        ...
        router.push(
          `/verify-email?email=${encodeURIComponent(result.email || formData.usernameOrEmail)}`
        );
      } else {
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Link
                  ...
                  className="text-sm text-purple-600 dark:text-purple-400 hover:text-purple-700 dark:hover:text-purple-300 animated-link"
                >
              ...
              <div className="relative">
                <input
                  ...
                  onChange={handleChange}
                  required
                  className="glass-input w-full px-4 py-3 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all duration-200 pr-10"
