      ...
      <div className="relative z-10 min-h-screen flex items-center justify-center p-4">
        <div className="glass-premium-card rounded-3xl p-6 sm:p-8 w-full max-w-md animate-glass-fade-in">
          <div className="text-center mb-6 sm:mb-8">
            <h1 className="text-2xl sm:text-3xl font-bold gradient-text mb-2">{t('auth', 'welcomeBack')}</h1>
            <p className="text-sm sm:text-base text-gray-600 dark:text-gray-300">{t('auth', 'smartUniversityAccount')}</p>
          </div>
...
export default function LoginPage() {
  const { t } = useI18n()
  return (
    <Suspense fallback={<div>{t('auth', 'loading')}</div>}>
      <LoginForm />
    </Suspense>
  )
}