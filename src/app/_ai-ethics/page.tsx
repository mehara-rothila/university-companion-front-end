...
export default function AIEthicsPage() {
  ...
  const [aiDecisions] = useState<AIDecision[]>([
    {
      ...
      factorsUsed: [
        { id: '1', name: 'Study Pattern', value: 'Afternoon Preference', weight: 30, description: 'You typically study better in afternoon hours', isPersonal: true },
        { id: '2', name: 'Noise Level', value: 'Quiet', weight: 25, description: 'Historical preference for quiet study environments', isPersonal: true },
        { id: '3', name: 'Current Occupancy', value: '23%', weight: 20, description: 'Real-time occupancy data for library sections', isPersonal: false },
        { id: '4', name: 'Academic Subject', value: 'Mathematics', weight: 15, description: 'Subject being studied affects space recommendation', isPersonal: true },
        { id: '5', name: 'Success Rate', value: '92%', weight: 10, description: 'Previous success rate at similar locations', isPersonal: true }
      ],
      outcome: 'Accepted',
      userImpact: 'medium',
      canAppeal: true,
    ...
    },
    {
      ...
      factorsUsed: [
        { id: '6', name: 'Focus Duration', value: '110 min avg', weight: 35, description: 'Your average sustained focus time', isPersonal: true },
        { id: '7', name: 'Exam Schedule', value: '3 days', weight: 30, description: 'Time until next calculus exam', isPersonal: true },
        { id: '8', name: 'Current Progress', value: '67%', weight: 20, description: 'Completion rate for current chapter', isPersonal: true },
        { id: '9', name: 'Time of Day', value: 'Morning', weight: 15, description: 'Peak performance time based on historical data', isPersonal: true }
      ],
      outcome: 'Accepted',
      userImpact: 'high',
      canAppeal: true,
    ...
    },
    {
      ...
      factorsUsed: [
        { id: '10', name: 'Stress Indicators', value: 'Elevated', weight: 40, description: 'Based on usage patterns and self-reported mood', isPersonal: true },
        { id: '11', name: 'Screen Time', value: '4.2 hours', weight: 30, description: 'Continuous screen time without break', isPersonal: true },
        { id: '12', name: 'Previous Effectiveness', value: '85%', weight: 20, description: 'Success rate of mindfulness recommendations', isPersonal: true },
        { id: '13', name: 'Time Available', value: '30 min', weight: 10, description: 'Break time available in current schedule', isPersonal: true }
      ],
      outcome: 'Declined',
      userImpact: 'low',
      canAppeal: true,
  ...
  const [biasReports] = useState<BiasReport[]>([
    ...
    },
    {
      ...
      severity: 'low',
      description: 'System recommended fewer advanced challenges to first-year students regardless of ability',
    ...
    },
    {
      ...
      severity: 'high',
      description: 'Team formation algorithm showed imbalanced gender distribution in technical challenges',
  ...
  const toggleOptOut = (optionId: string) => {
    setOptOutOptions(prev => prev.map(option => 
      option.id === optionId 
        ? { ...option, isOptedOut: !option.isOptedOut }
        : option
    ));
  };
      ...
      <>
        ...
        <main className={`min-h-screen ${isDarkMode ? 'bg-gradient-to-b from-gray-900 to-gray-800' : 'bg-gradient-to-b from-gray-50 to-gray-100'} transition-colors duration-300 flex items-center justify-center`}>
          <div className="text-center">
            ...
            <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Loading AI ethics data...</p>
          </div>
        </main>
      </>
    );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10 pt-24">
          ...
          <div className="mb-8 animate-fade-in">
            <div className="text-center">
              ...
              <p className={`text-xl ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} max-w-3xl mx-auto mb-6`}>
                Complete transparency into AI decision-making with bias detection, algorithmic accountability, and user control mechanisms.
              </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
            {ethicsMetrics.map((metric) => (
              <div key={metric.id} className={`p-4 rounded-xl ${isDarkMode ? 'bg-purple-900/20 border border-purple-800' : 'bg-purple-50 border border-purple-200'} text-center animate-fade-in`}>
                <div className={`text-2xl font-bold mb-1 ${
                  metric.value >= metric.target ? 'text-green-600 dark:text-green-400' : 'text-orange-600 dark:text-orange-400'
                }`}>
                  {metric.value}%
                </div>
                <div className={`text-xs ${isDarkMode ? 'text-purple-300' : 'text-purple-700'} mb-1`}>{metric.name}</div>
          ...
          <div className={`mb-8 ${isDarkMode ? 'bg-green-900/20 border-green-800' : 'bg-green-50 border-green-200'} rounded-2xl p-6 border animate-fade-in`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800/50' : 'bg-white/50'} border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                <h4 className={`font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-2`}>✅ Transparent AI</h4>
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  All AI decisions include explanations and can be reviewed by users
                </p>
              </div>
              <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800/50' : 'bg-white/50'} border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                <h4 className={`font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-2`}>⚖️ Bias Monitoring</h4>
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                  Continuous bias detection with immediate mitigation protocols
                </p>
              </div>
              <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-800/50' : 'bg-white/50'} border ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                <h4 className={`font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-2`}>🎛️ User Control</h4>
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
          ...
          <div className={`${isDarkMode ? 'bg-gray-800/90 border-gray-700' : 'bg-white/90 border-gray-100'} rounded-2xl shadow-lg border backdrop-blur-sm animate-fade-in`}>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                    ...
                    <div className="space-y-4">
                      <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-600/50' : 'bg-white'} border ${isDarkMode ? 'border-gray-600' : 'border-gray-200'}`}>
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>AI Decisions Made</p>
                          </div>
                          <div>
                          ...
                          <div>
                            ...
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Appeal Rate</p>
                          </div>
                          <div>
                            <span className={`text-2xl font-bold ${isDarkMode ? 'text-purple-400' : 'text-purple-600'}`}>
                              {transparencyReport.complianceScore}%
                            </span>
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Compliance Score</p>
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {aiDecisions.map((decision) => (
                    <div key={decision.id} className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'}`}>
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <div className="flex items-center space-x-3 mb-2">
                            ...
                            <span className={`text-xs px-2 py-1 rounded-full ${getImpactColor(decision.userImpact)}`}>
                        ...
                        <div className="text-right">
                          <div className={`text-lg font-bold ${decision.confidence >= 80 ? 'text-green-500' : decision.confidence >= 60 ? 'text-yellow-500' : 'text-red-500'}`}>
                            {decision.confidence}%
                          </div>
                      ...
                      <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-blue-900/20 border border-blue-800' : 'bg-blue-50 border border-blue-200'} mb-4`}>
                        <h4 className={`text-sm font-medium ${isDarkMode ? 'text-blue-300' : 'text-blue-800'} mb-2`}>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="space-y-2">
                          {decision.factorsUsed.map((factor) => (
                            <div key={factor.id} className={`flex items-center justify-between p-3 rounded ${isDarkMode ? 'bg-gray-600/50' : 'bg-white'}`}>
              ...
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {biasReports.map((report) => (
                    <div key={report.id} className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'}`}>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {report.affectedGroups.map(group => (
                            <span key={group} className={`px-2 py-1 text-xs rounded ${isDarkMode ? 'bg-red-900/30 text-red-300' : 'bg-red-100 text-red-700'}`}>
                              {group}
                            </span>
                          ))}
                        </div>
              ...
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {algorithmUpdates.map((update) => (
                    <div key={update.id} className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'}`}>
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <div className="flex items-center space-x-3 mb-2">
                            ...
                            <span className={`text-xs px-2 py-1 rounded-full ${getImpactColor(update.impactLevel)}`}>
                              {update.impactLevel} impact
                            </span>
                            <span className={`text-xs px-2 py-1 rounded-full ${
                              update.updateType === 'bias-mitigation' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' :
                              update.updateType === 'security' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' :
                              update.updateType === 'feature' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' :
                      ...
                      <div className="grid grid-cols-3 gap-4 mb-4">
                        <div className={`p-3 rounded ${isDarkMode ? 'bg-gray-600/50' : 'bg-white'} text-center`}>
                          <div className={`text-lg font-bold ${update.performanceChange.accuracy >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                            {update.performanceChange.accuracy >= 0 ? '+' : ''}{update.performanceChange.accuracy}%
                          </div>
                          <div className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Accuracy</div>
                        </div>
                        <div className={`p-3 rounded ${isDarkMode ? 'bg-gray-600/50' : 'bg-white'} text-center`}>
                          <div className={`text-lg font-bold ${update.performanceChange.fairness >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                            {update.performanceChange.fairness >= 0 ? '+' : ''}{update.performanceChange.fairness}%
                          </div>
                          <div className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Fairness</div>
                        </div>
                        <div className={`p-3 rounded ${isDarkMode ? 'bg-gray-600/50' : 'bg-white'} text-center`}>
                          <div className={`text-lg font-bold ${update.performanceChange.speed >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                            {update.performanceChange.speed >= 0 ? '+' : ''}{update.performanceChange.speed}%
                          </div>
                          <div className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Speed</div>
                        </div>
                      </div>
                      ...
                      <div className="mb-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {update.affectedFeatures.map(feature => (
                            <span key={feature} className={`px-2 py-1 text-xs rounded ${isDarkMode ? 'bg-blue-900/30 text-blue-300' : 'bg-blue-100 text-blue-700'}`}>
                              {feature}
                            </span>
                          ))}
                        </div>
            ...
            {activeTab === 'data-usage' && (
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {dataUsage.map((usage) => (
                    <div key={usage.id} className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'}`}>
                      ...
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="flex items-center space-x-2">
                          ...
                          <span className={`text-xs px-2 py-1 rounded-full ${
                            usage.sharingStatus === 'not-shared' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-300' :
                            usage.sharingStatus === 'internal' ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-300' :
                            'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300'
                          }`}>
                            {usage.sharingStatus}
                          </span>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        ...
                        <div className="space-y-2">
                          <div>
                            <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Retention:</span>
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{usage.retentionPeriod}</p>
                          </div>
                          <div>
                            <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Last Accessed:</span>
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{usage.lastAccessed.toLocaleDateString()}</p>
                          </div>
                          <div>
                            <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Access Frequency:</span>
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{usage.accessFrequency} times this month</p>
                          </div>
                        </div>
                      </div>
            ...
            {activeTab === 'controls' && (
              <div className="p-8">
                ...
                <div className="space-y-6">
                  {['recommendations', 'personalization', 'analytics', 'research'].map((category) => (
                    <div key={category} className="mb-8">
                      <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'} mb-4 capitalize`}>
                        {category}
                      </h3>
                      
                      <div className="space-y-4">
                        {optOutOptions.filter(option => option.category === category).map((option) => (
                          <div key={option.id} className={`p-6 rounded-xl ${isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'}`}>
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                {/* Impact */}
                                <div className={`p-3 rounded ${isDarkMode ? 'bg-yellow-900/20 border border-yellow-800' : 'bg-yellow-50 border border-yellow-200'} mb-3`}>
                                  <h5 className={`text-sm font-medium ${isDarkMode ? 'text-yellow-300' : 'text-yellow-800'} mb-1`}>
                                  ...
                                  <div className="mb-3">
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                      {option.dependencies.map(dep => (
                                        <span key={dep} className={`px-2 py-1 text-xs rounded ${isDarkMode ? 'bg-gray-600 text-gray-300' : 'bg-gray-200 text-gray-600'}`}>
                                          {dep}
                                        </span>
                                      ))}
                                    </div>
            ...
            <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto`}>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {selectedDecision.factorsUsed.map((factor) => (
                      <div key={factor.id} className={`p-3 rounded ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className={`font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Confidence:</span>
                    <p className={`${selectedDecision.confidence >= 80 ? 'text-green-500' : 'text-yellow-500'}`}>
                      {selectedDecision.confidence}%
                    </p>
                  </div>
                  <div>
                    <span className={`font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Impact:</span>
            ...
            <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-xl max-w-md w-full p-6`}>
              ...
              <div className="text-center">
                ...
                <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mb-6`}>
                  Your appeal will be reviewed by our ethics team within 24 hours. Please explain why you believe this decision was incorrect.
                </p>
