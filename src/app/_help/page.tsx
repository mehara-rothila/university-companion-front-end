...
export default function HelpSupportPage() {
  ...
  const [faqItems] = useState<FAQItem[]>([
    ...
    },
    {
      ...
      answer: 'Yes, absolutely. Wellness check-ins and mood data are encrypted and stored securely. This information is never shared with anyone without your explicit consent. You can view, edit, or delete your wellness data anytime in the Privacy Center.',
      helpful: 44,
    ...
    },
    {
      ...
      answer: 'Try these steps: 1) Close and restart the app, 2) Check your internet connection, 3) Clear the app cache in settings, 4) Restart your device, 5) Update to the latest app version. If problems persist, contact our technical support.',
      helpful: 26,
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10 pt-24">
          ...
          <div className={`${isDarkMode ? 'bg-gray-800/90 border-gray-700' : 'bg-white/90 border-gray-100'} rounded-2xl shadow-lg border backdrop-blur-sm p-8 animate-fade-in`}>
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  {filteredFAQs.map((faq) => (
                    <div key={faq.id} className={`p-6 rounded-lg ${isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'} transition-all duration-200 hover:shadow-md`}>
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1">
                          <h3 className={`font-semibold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-2`}>{faq.question}</h3>
                          <span className={`inline-block px-2 py-1 text-xs rounded-full ${isDarkMode ? 'bg-purple-900/30 text-purple-300' : 'bg-purple-100 text-purple-700'}`}>
                            {faq.category}
                          </span>
                        </div>
                      </div>
                      <p className={`${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-4 leading-relaxed`}>{faq.answer}</p>
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          <span className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Was this helpful?</span>
                          ...
                          <span className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>{faq.helpful}</span>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                {filteredFAQs.length === 0 && (
                  <div className="text-center py-12">
                    ...
                    <p className={`text-lg ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>No FAQs found matching your search.</p>
                    <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-400'} mt-2`}>Try adjusting your search terms or category filter.</p>
                  </div>
                )}
              </div>
            )}
            ...
            {activeSection === 'tutorials' && (
              <div className="space-y-6">
                <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-6`}>Video Tutorials & Guides</h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {tutorials.map((tutorial) => (
                    <div key={tutorial.id} className={`p-6 rounded-lg ${isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'} transition-all duration-200 hover:shadow-md`}>
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <h3 className={`font-semibold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-2`}>{tutorial.title}</h3>
                          <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mb-3`}>{tutorial.description}</p>
                        </div>
                      </div>
                      
                      <div className="flex items-center space-x-4 mb-4">
                        <span className={`px-2 py-1 text-xs rounded-full ${isDarkMode ? 'bg-blue-900/30 text-blue-300' : 'bg-blue-100 text-blue-700'}`}>
                          {tutorial.duration}
                        </span>
                        ...
                        <span className={`px-2 py-1 text-xs rounded-full ${isDarkMode ? 'bg-purple-900/30 text-purple-300' : 'bg-purple-100 text-purple-700'}`}>
                          {tutorial.category}
                        </span>
                      </div>

                      <div className="mb-4">
                        <h4 className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>What you&apos;ll learn:</h4>
                        <ul className="space-y-1">
                          {tutorial.steps.slice(0, 3).map((step, index) => (
                            <li key={index} className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} flex items-center`}>
                              <span className="w-1 h-1 bg-purple-500 rounded-full mr-2"></span>
                              {step}
                            </li>
                          ))}
                          {tutorial.steps.length > 3 && (
                            <li className={`text-xs ${isDarkMode ? 'text-gray-500' : 'text-gray-500'} italic`}>
            ...
            {activeSection === 'contact' && (
              <div className="space-y-6">
                <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-6`}>Contact Support</h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {contactMethods.map((method) => (
                    <div key={method.type} className={`p-6 rounded-lg ${isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'} transition-all duration-200 hover:shadow-md`}>
                      <div className="flex items-start">
                        ...
                        <div className="flex-1">
                          <h3 className={`font-semibold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-2`}>{method.title}</h3>
            ...
            {activeSection === 'tickets' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>Support Tickets</h2>
                  <button 
                    onClick={() => setShowTicketForm(true)}
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-blue-900/20 border border-blue-800' : 'bg-blue-50 border border-blue-200'}`}>
                    ...
                    <div className={`text-sm ${isDarkMode ? 'text-blue-300' : 'text-blue-700'}`}>Open</div>
                  </div>
                  <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-yellow-900/20 border border-yellow-800' : 'bg-yellow-50 border border-yellow-200'}`}>
                    ...
                    <div className={`text-sm ${isDarkMode ? 'text-yellow-300' : 'text-yellow-700'}`}>In Progress</div>
                  </div>
                  <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-green-900/20 border border-green-800' : 'bg-green-50 border border-green-200'}`}>
                    ...
                    <div className={`text-sm ${isDarkMode ? 'text-green-300' : 'text-green-700'}`}>Resolved</div>
                  </div>
                  <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'}`}>
                    <div className={`text-2xl font-bold ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>11</div>
                ...
                {/* Recent Tickets */}
                <div>
                  <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'} mb-4`}>Recent Tickets</h3>
                  <div className="space-y-3">
                    {[
                      { id: '#2024-001', subject: 'AI chatbot not responding', status: 'in-progress', priority: 'medium', date: '2 hours ago' },
                      { id: '#2024-002', subject: 'Study space booking error', status: 'resolved', priority: 'low', date: '1 day ago' },
                      { id: '#2024-003', subject: 'Navigation not loading', status: 'open', priority: 'high', date: '3 days ago' }
                    ].map((ticket) => (
                      <div key={ticket.id} className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/50 border border-gray-600' : 'bg-gray-50 border border-gray-200'} transition-all duration-200 hover:shadow-md`}>
                        <div className="flex items-center justify-between">
                          <div className="flex-1">
                            <div className="flex items-center space-x-3 mb-2">
                              <span className={`font-medium ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>{ticket.id}</span>
                              <span className={`px-2 py-1 text-xs rounded-full ${
                              ...
                              <span className={`px-2 py-1 text-xs rounded-full ${
                                ticket.priority === 'high' ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' :
                                ticket.priority === 'medium' ? 'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-300' :
                                'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300'
                              }`}>
                                {ticket.priority}
                              </span>
                            </div>
                            <h4 className={`font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-800'} mb-1`}>{ticket.subject}</h4>
                            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>{ticket.date}</p>
                          </div>
                          <button className={`text-sm ${isDarkMode ? 'text-purple-400 hover:text-purple-300' : 'text-purple-600 hover:text-purple-700'} transition-colors duration-200`}>
                            View Details
                          </button>
                        </div>
                      </div>
            ...
            {activeSection === 'feedback' && (
              <div className="space-y-6">
                <h2 className={`text-2xl font-bold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-6`}>Send Feedback</h2>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {/* Feedback Form */}
                  <div>
                    <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'} mb-4`}>Share Your Thoughts</h3>
                    <div className="space-y-4">
                      <div>
                        <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Feedback Type</label>
                      ...
                      <div>
                        <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Subject</label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Details</label>
                        <textarea
                          rows={6}
                  ...
                  {/* Feedback Stats */}
                  <div>
                    <h3 className={`text-lg font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-800'} mb-4`}>Community Feedback</h3>
                    <div className="space-y-4">
                      <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                        <div className="flex items-center justify-between mb-2">
                          <span className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>Overall Satisfaction</span>
                      ...
                      <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                        <h4 className={`font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-800'} mb-3`}>Recent Improvements</h4>
                        <ul className="space-y-2">
                          <li className={`text-sm ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} flex items-center`}>
                      ...
                      <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-purple-900/20 border border-purple-800' : 'bg-purple-50 border border-purple-200'}`}>
                        <h4 className={`font-medium ${isDarkMode ? 'text-purple-300' : 'text-purple-800'} mb-2`}>Feature Voting</h4>
                        <p className={`text-sm ${isDarkMode ? 'text-purple-400' : 'text-purple-700'} mb-3`}>
                          Help us prioritize new features by voting on what matters most to you!
                        </p>
                        <button className={`text-sm ${isDarkMode ? 'text-purple-400 hover:text-purple-300' : 'text-purple-600 hover:text-purple-700'} transition-colors duration-200 flex items-center`}>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto`}>
              <div className="flex items-center justify-between mb-6">
                <h2 className={`text-xl font-bold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}>Submit Support Ticket</h2>
                <button 
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Priority</label>
                    <select className={`w-full px-4 py-3 rounded-lg border transition-all duration-200 ${
                      isDarkMode 
                        ? 'bg-gray-700 border-gray-600 text-gray-100' 
                        : 'bg-white border-gray-300 text-gray-900'
