...
export default function AdminLibraryPage() {
    ...
    const getCategoryColor = (category: string) => {
        switch (category) {
            case 'PROGRAMMING': return 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300';
            case 'ENGINEERING': return 'bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300';
            case 'TEXTBOOK': return 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300';
            case 'REFERENCE': return 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300';
            default: return 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300';
        }
    };
                    ...
                    <div className="mb-8">
                        <div className={`flex flex-col md:flex-row md:items-center justify-between p-6 rounded-xl ${isDarkMode ? 'bg-gray-900/80' : 'bg-white/80'} backdrop-blur-sm shadow-lg gap-4`}>
                            ...
                            <div className="flex items-center space-x-4">
                                ...
                                <div className={`px-4 py-2 rounded-lg ${isDarkMode ? 'bg-indigo-900/30 border border-indigo-700/30' : 'bg-indigo-100 border border-indigo-200'}`}>
                    ...
                    <div className={`${isDarkMode ? 'bg-gray-800/90 border-gray-700' : 'bg-white/90 border-gray-100'} rounded-2xl shadow-lg border backdrop-blur-sm p-6`}>
                        ...
                        {loading ? (
                            <div className="text-center py-12">
                                ...
                                <p className={`${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>Loading books...</p>
                            </div>
                        ) : books.length === 0 ? (
                            <div className="text-center py-12">
                                <BookOpen className={`w-16 h-16 mx-auto mb-4 ${isDarkMode ? 'text-gray-500/50' : 'text-gray-400/50'}`} />
                                <p className={`text-xl ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                                    {filter === 'PENDING' ? 'No pending books' :
                                        filter === 'APPROVED' ? 'No approved books' :
                                            filter === 'REJECTED' ? 'No rejected books' :
                                                'No books found'}
                                </p>
                                    ...
                                    >
                                        <div className="flex justify-between items-start mb-4">
                                            <div className="flex items-center space-x-2 flex-wrap gap-1">
                                                <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${book.bookType === 'PHYSICAL'
                                                    ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300'
                                                    : 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300'
                                                    }`}>
                                                    {book.bookType === 'PHYSICAL' ? '📚 Physical' : '📄 Digital'}
                                                </span>
                                                <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${getStatusColor(book.status)}`}>
                                                    {book.status}
                                                </span>
                                                <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${getCategoryColor(book.category)}`}>
                                                    {book.category}
                                                </span>
                                            </div>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs">
                                                    {book.bookType === 'PHYSICAL' && book.bookCondition && (
                                                        <span className={`px-2 py-0.5 rounded-full ${getConditionColor(book.bookCondition)}`}>
                                                            {book.bookCondition}
                                                        </span>
                                                    )}
                                                    ...
                                                    {book.price && book.price > 0 && (
                                                        <span className={`flex items-center ${isDarkMode ? 'text-green-300' : 'text-green-700'}`}>
                                                ...
                                                <div className={`flex items-center mt-2 text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>
                                                    ...
                                                    <span>Uploaded by: {book.ownerName} ({book.ownerEmail})</span>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                        <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-xl max-w-md w-full p-6`}>
                            ...
                            <div className={`flex items-center gap-3 p-3 rounded-lg mb-4 ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-100'}`}>
                                <div className={`w-12 h-12 rounded-lg flex items-center justify-center ${isDarkMode ? 'bg-gray-600' : 'bg-gray-200'}`}>
                                    {selectedBook.bookType === 'DIGITAL' ? (
                                        <FileText className={`w-6 h-6 ${isDarkMode ? 'text-purple-400' : 'text-purple-500'}`} />
                                    ) : (
                                        <BookOpen className={`w-6 h-6 ${isDarkMode ? 'text-blue-400' : 'text-blue-500'}`} />
                                    )}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className={`font-medium truncate ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>{selectedBook.title}</p>
                                    <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>by {selectedBook.author} • {selectedBook.category}</p>
                                </div>
                            </div>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                        <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-xl max-w-md w-full p-6`}>
                            ...
                            <p className={`text-sm ${isDarkMode ? 'text-red-400' : 'text-red-600'} mb-6`}>
                                ⚠️ This action cannot be undone. The book will be permanently removed from the database.
                            </p>
                            <div className="flex space-x-3">
                                <button
