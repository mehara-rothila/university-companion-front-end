...
export default function AdminPanel() {
    ...
    try {
      ...
      try {
        ...
        setModuleCounts((prev) => ({
          ...prev,
          pendingAchievements: achievementsRes.data?.length || 0,
        }));
      } catch (err) {
      ...
      try {
        const financialAidRes = await axios.get(
          `${API_BASE}/admin/financial-aid/applications?status=PENDING`,
          {
            headers: getAuthHeaders(),
          }
        );
        setModuleCounts((prev) => ({
          ...prev,
          pendingFinancialAid: financialAidRes.data?.length || 0,
        }));
      } catch (err) {
      ...
      try {
        const notificationsRes = await axios.get(
          `${API_BASE}/notifications/admin/all?page=0&size=1`,
          {
            headers: getAuthHeaders(),
          }
        );
        setModuleCounts((prev) => ({
          ...prev,
          activeNotifications: notificationsRes.data?.totalElements || 0,
        }));
      } catch (err) {
      ...
      try {
        ...
        setModuleCounts((prev) => ({
          ...prev,
          pendingCompetitions: competitionsRes.data?.length || 0,
        }));
      } catch (err) {
  ...
  const performBulkAction = async (action: string) => {
    ...
    if (!confirm(t('admin.messages.confirmBulkAction', { action, count: selectedUsers.length })))
      return;

    try {
      await axios.post(
        `${API_BASE}/admin/users/bulk-action`,
        {
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10 pt-24">
            ...
            >
              ...
              >
                <svg
                  ...
                  className="h-10 w-10 mr-3 text-red-500"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8 animate-fade-in">
            {/* User Management */}
            <div
              className={`glass-card ${isDarkMode ? 'bg-gradient-to-br from-blue-900/20 to-blue-800/20 border-blue-700/30' : 'bg-gradient-to-br from-blue-50/90 to-blue-100/90 border-blue-200/50'} backdrop-blur-lg border p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] hover:-translate-y-1 cursor-pointer h-full flex flex-col`}
            >
              <div className="flex items-start justify-between mb-4">
                <div
                  className={`p-3 rounded-lg ${isDarkMode ? 'bg-blue-800/50' : 'bg-blue-100'} flex-shrink-0`}
                >
                  <Users className={`w-8 h-8 ${isDarkMode ? 'text-blue-300' : 'text-blue-600'}`} />
                </div>
                <div className="text-right">
                  <span
                    className={`text-2xl font-bold ${isDarkMode ? 'text-blue-300' : 'text-blue-600'} block`}
                  >
                    {stats?.totalUsers || 0}
                  </span>
              ...
              <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
                <span
                  className={`text-sm font-medium ${isDarkMode ? 'text-blue-300' : 'text-blue-600'} flex items-center justify-end`}
                >
            ...
            <Link href="/admin/financial-aid">
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="text-right">
                    <span
                      className={`text-2xl font-bold ${isDarkMode ? 'text-green-300' : 'text-green-600'} block`}
                    >
                ...
                <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
                  <span
                    className={`text-sm font-medium ${isDarkMode ? 'text-green-300' : 'text-green-600'} flex items-center justify-end`}
                  >
            ...
            <Link href="/admin/notifications">
              <div
                className={`glass-card ${isDarkMode ? 'bg-gradient-to-br from-amber-900/20 to-amber-800/20 border-amber-700/30' : 'bg-gradient-to-br from-amber-50/90 to-amber-100/90 border-amber-200/50'} backdrop-blur-lg border p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] hover:-translate-y-1 cursor-pointer h-full flex flex-col`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`p-3 rounded-lg ${isDarkMode ? 'bg-amber-800/50' : 'bg-amber-100'} flex-shrink-0`}
                  >
                    <Bell
                      className={`w-8 h-8 ${isDarkMode ? 'text-amber-300' : 'text-amber-600'}`}
                    />
                  </div>
                  <div className="text-right">
                    <span
                      className={`text-2xl font-bold ${isDarkMode ? 'text-amber-300' : 'text-amber-600'} block`}
                    >
                ...
                <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
                  <span
                    className={`text-sm font-medium ${isDarkMode ? 'text-amber-300' : 'text-amber-600'} flex items-center justify-end`}
                  >
            ...
            <Link href="/admin/emergency">
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="text-right">
                    <span
                      className={`text-sm font-semibold ${isDarkMode ? 'text-red-300' : 'text-red-600'} bg-gradient-to-r ${isDarkMode ? 'from-red-400 to-red-300' : 'from-red-600 to-red-500'} bg-clip-text text-transparent block`}
                    >
                      URGENT
                    </span>
                ...
                <div className="flex-grow">
                  ...
                  <p
                    className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mb-4 leading-relaxed`}
                  >
                    Send urgent emergency notifications to all students or specific users with
                    real-time delivery and acknowledgment tracking
                  </p>
                </div>
                <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
                  <span
                    className={`text-sm font-medium ${isDarkMode ? 'text-red-300' : 'text-red-600'} flex items-center justify-end`}
                  >
            ...
            <Link href="/admin/competitions">
              <div
                className={`glass-card ${isDarkMode ? 'bg-gradient-to-br from-orange-900/20 to-orange-800/20 border-orange-700/30' : 'bg-gradient-to-br from-orange-50/90 to-orange-100/90 border-orange-200/50'} backdrop-blur-lg border p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] hover:-translate-y-1 cursor-pointer h-full flex flex-col`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`p-3 rounded-lg ${isDarkMode ? 'bg-orange-800/50' : 'bg-orange-100'} flex-shrink-0`}
                  >
                    <Trophy
                      className={`w-8 h-8 ${isDarkMode ? 'text-orange-300' : 'text-orange-600'}`}
                    />
                  </div>
                  <div className="text-right">
                    <span
                      className={`text-2xl font-bold ${isDarkMode ? 'text-orange-300' : 'text-orange-600'} block`}
                    >
                      {moduleCounts.pendingCompetitions}
                    </span>
                    <span
                      className={`text-xs ${isDarkMode ? 'text-orange-400' : 'text-orange-500'}`}
                    >
                      Pending
                    </span>
                  </div>
                ...
                <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
                  <span
                    className={`text-sm font-medium ${isDarkMode ? 'text-orange-300' : 'text-orange-600'} flex items-center justify-end`}
                  >
            ...
            <Link href="/admin/lost-found">
              <div
                className={`glass-card ${isDarkMode ? 'bg-gradient-to-br from-cyan-900/20 to-cyan-800/20 border-cyan-700/30' : 'bg-gradient-to-br from-cyan-50/90 to-cyan-100/90 border-cyan-200/50'} backdrop-blur-lg border p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] hover:-translate-y-1 cursor-pointer h-full flex flex-col`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`p-3 rounded-lg ${isDarkMode ? 'bg-cyan-800/50' : 'bg-cyan-100'} flex-shrink-0`}
                  >
                    <Search
                      className={`w-8 h-8 ${isDarkMode ? 'text-cyan-300' : 'text-cyan-600'}`}
                    />
                  </div>
                  <div className="text-right">
                    <span
                      className={`text-2xl font-bold ${isDarkMode ? 'text-cyan-300' : 'text-cyan-600'} block`}
                    >
                ...
                <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
                  <span
                    className={`text-sm font-medium ${isDarkMode ? 'text-cyan-300' : 'text-cyan-600'} flex items-center justify-end`}
                  >
                    Manage Items
                    <ChevronRight className="w-4 h-4 ml-1" />
                  </span>
            ...
            <Link href="/admin/library">
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="text-right">
                    <span
                      className={`text-2xl font-bold ${isDarkMode ? 'text-indigo-300' : 'text-indigo-600'} block`}
                    >
                      {moduleCounts.pendingLibraryBooks}
                    </span>
                    <span
                      className={`text-xs ${isDarkMode ? 'text-indigo-400' : 'text-indigo-500'}`}
                    >
                      Pending
                    </span>
                  </div>
            ...
            <Link href="/admin/events">
              <div
                className={`glass-card ${isDarkMode ? 'bg-gradient-to-br from-pink-900/20 to-pink-800/20 border-pink-700/30' : 'bg-gradient-to-br from-pink-50/90 to-pink-100/90 border-pink-200/50'} backdrop-blur-lg border p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] hover:-translate-y-1 cursor-pointer h-full flex flex-col`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`p-3 rounded-lg ${isDarkMode ? 'bg-pink-800/50' : 'bg-pink-100'} flex-shrink-0`}
                  >
                    <Calendar
                      className={`w-8 h-8 ${isDarkMode ? 'text-pink-300' : 'text-pink-600'}`}
                    />
                  </div>
                  <div className="text-right">
                    <span
                      className={`text-2xl font-bold ${isDarkMode ? 'text-pink-300' : 'text-pink-600'} block`}
                    >
                ...
                <div className="flex-grow">
                  ...
                  <p
                    className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mb-4 leading-relaxed`}
                  >
                    Review and approve pending events, manage event submissions, and moderate social
                    activities
                  </p>
                </div>
                <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
                  <span
                    className={`text-sm font-medium ${isDarkMode ? 'text-pink-300' : 'text-pink-600'} flex items-center justify-end`}
                  >
                    Manage Events
                    <ChevronRight className="w-4 h-4 ml-1" />
                  </span>
            ...
            <Link href="/admin/achievements">
              <div
                className={`glass-card ${isDarkMode ? 'bg-gradient-to-br from-yellow-900/20 to-yellow-800/20 border-yellow-700/30' : 'bg-gradient-to-br from-yellow-50/90 to-yellow-100/90 border-yellow-200/50'} backdrop-blur-lg border p-6 rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] hover:-translate-y-1 cursor-pointer h-full flex flex-col`}
              >
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`p-3 rounded-lg ${isDarkMode ? 'bg-yellow-800/50' : 'bg-yellow-100'} flex-shrink-0`}
                  >
                    <Trophy
                      className={`w-8 h-8 ${isDarkMode ? 'text-yellow-300' : 'text-yellow-600'}`}
                    />
                  </div>
                  <div className="text-right">
                    <span
                      className={`text-2xl font-bold ${isDarkMode ? 'text-yellow-300' : 'text-yellow-600'} block`}
                    >
                      {moduleCounts.pendingAchievements}
                    </span>
                    <span
                      className={`text-xs ${isDarkMode ? 'text-yellow-400' : 'text-yellow-500'}`}
                    >
                      Pending
                    </span>
                  </div>
                ...
                <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
                  <span
                    className={`text-sm font-medium ${isDarkMode ? 'text-yellow-300' : 'text-yellow-600'} flex items-center justify-end`}
                  >
                    Manage Achievements
                    <ChevronRight className="w-4 h-4 ml-1" />
                  </span>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div
                  className={`p-3 rounded-lg ${isDarkMode ? 'bg-purple-800/50' : 'bg-purple-100'} flex-shrink-0`}
                >
                  <Settings
                ...
                <div className="text-right">
                  <span
                    className={`text-sm font-semibold ${isDarkMode ? 'text-purple-300' : 'text-purple-600'} bg-gradient-to-r ${isDarkMode ? 'from-purple-400 to-purple-300' : 'from-purple-600 to-purple-500'} bg-clip-text text-transparent block`}
                  >
              ...
              <div className="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
                <span
                  className={`text-sm font-medium ${isDarkMode ? 'text-purple-300' : 'text-purple-600'} flex items-center justify-end opacity-60`}
                >
          ...
          {/* Quick Actions */}
          <div
            className={`glass-card ${isDarkMode ? 'bg-gray-800/90 border-gray-700' : 'bg-white/90 border-gray-100'} rounded-2xl shadow-lg p-6 border backdrop-blur-sm mb-8 animate-fade-in`}
          >
            <h2
              className={`text-xl font-semibold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-4`}
            >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              >
                ...
                <span
                  className={`text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} text-center`}
                >
              ...
              <Link href="/admin/financial-aid">
                <div
                  className={`p-4 rounded-lg border-2 border-dashed transition-all duration-200 hover:scale-105 cursor-pointer flex flex-col items-center text-center ${
                    isDarkMode
              ...
              >
                ...
                <span
                  className={`text-sm font-medium ${isDarkMode ? 'text-gray-500' : 'text-gray-400'} text-center`}
                >
          ...
          >
            <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6">
              <h2
                className={`text-xl font-semibold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-4 lg:mb-0`}
              >
              ...
              <div className="flex flex-col sm:flex-row gap-4">
                ...
                <div className="relative dropdown-container z-20">
                  <button
                    ...
                    className={`glass-input px-4 py-2 pr-8 rounded-lg transition-all duration-200 text-left min-w-[120px] relative ${
                  ...
                  >
                    {roleFilter === ''
                      ...
                      : roleFilter === 'STUDENT'
                        ? t('admin.userManagement.filters.students')
                        : roleFilter === 'FACULTY'
                          ? t('admin.userManagement.filters.faculty')
                          : t('admin.userManagement.filters.admins')}
                    <span
                      className={`absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
                    >
                      <svg
                        className="w-4 h-4"
                        fill="none"
            ...
            {selectedUsers.length > 0 && (
              <div
                className={`flex items-center gap-4 p-4 rounded-lg mb-4 ${isDarkMode ? 'bg-blue-900/20 border border-blue-800' : 'bg-blue-50 border border-blue-200'}`}
              >
                <span
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className={`sticky top-0 z-10 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
                  <tr className={`border-b ${isDarkMode ? 'border-gray-700' : 'border-gray-200'}`}>
                    ...
                    <th
                      className={`text-left p-4 font-semibold ${isDarkMode ? 'text-gray-200' : 'text-gray-700'}`}
                    >
              ...
              <div
                className={`max-h-[400px] overflow-y-auto ${isDarkMode ? 'scrollbar-dark' : 'scrollbar-light'}`}
              >
                <table className="w-full">
                  <tbody>
                      ...
                      >
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                            {user.imageUrl && !failedImages.has(user.id) ? (
                              <img
                                src={
                                  user.imageUrl.includes('amazonaws.com')
                              ...
                              <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center border-2 border-purple-500 ${isDarkMode ? 'bg-gradient-to-br from-purple-600 to-purple-500' : 'bg-gradient-to-br from-purple-600 to-purple-500'}`}
                              >
                        ...
                        <td className="p-4">
                          <div className="flex flex-col gap-1">
                            <span
                              className={`px-2 py-1 text-xs rounded-full ${getRoleColor(user.role)} w-fit`}
                            >
                              {user.role}
                            </span>
                        ...
                        <td className="p-4">
                          ...
                          >
                            {user.studentId && (
                              <div>
                                {t('admin.userManagement.table.details.id')}: {user.studentId}
                              </div>
                            )}
                            {user.major && (
                              <div>
                                {t('admin.userManagement.table.details.major')}: {user.major}
                              </div>
                            )}
                            {user.year && (
                              <div>
                                {t('admin.userManagement.table.details.year')}: {user.year}
                              </div>
                            )}
                          </div>
                        </td>
                        <td className="p-4">
                          ...
                          >
                            {user.enabled
                              ? t('admin.userManagement.table.status.active')
                              : t('admin.userManagement.table.status.disabled')}
                          </span>
                        </td>
                        <td className="p-4">
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              title={
                                user.enabled
                                  ? t('admin.userManagement.table.actions.disable')
                                  : t('admin.userManagement.table.actions.enable')
                              }
                            >
                              <svg
                                className="h-4 w-4"
            ...
            <div className="flex items-center justify-between mt-6">
              <div className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                ...
                {(users?.currentPage || 0) * pageSize + 1} {t('admin.userManagement.pagination.to')}{' '}
                {Math.min(((users?.currentPage || 0) + 1) * pageSize, users?.totalItems || 0)}{' '}
                {t('admin.userManagement.pagination.of')} {users?.totalItems || 0}{' '}
                {t('admin.userManagement.pagination.users')}
              </div>
              <div className="flex items-center gap-2">
                <button
                ...
                >
                  {t('admin.userManagement.pagination.page')} {(users?.currentPage || 0) + 1}{' '}
                  {t('admin.userManagement.pagination.of')} {users?.totalPages || 1}
                </span>
                <button
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    >
                      {t('admin.modal.fields.firstName')}
                    </label>
                    <input
                      type="text"
                      required
                  ...
                  <div className="relative dropdown-container">
                    ...
                    >
                      {userForm.role === 'STUDENT'
                        ? t('admin.modal.roles.student')
                        : userForm.role === 'FACULTY'
                          ? t('admin.modal.roles.faculty')
                          : t('admin.modal.roles.admin')}
                      <div
                        className={`absolute inset-y-0 right-0 flex items-center px-2 pointer-events-none ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}
                      >
                        <svg
                        ...
                        {[
                          ...
                          <button
                            ...
                            onClick={() => {
                              setUserForm((prev) => ({
                                ...prev,
                                role: option.value as 'STUDENT' | 'FACULTY' | 'ADMIN',
                              }));
                ...
                {userForm.role === 'STUDENT' && (
                  <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                    ...
                    >
                      {t('admin.modal.sections.studentInfo')}
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <label
                          className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}
                        >
                          {t('admin.modal.fields.studentId')}
                        </label>
                        <input
                          type="text"
                      ...
                      <div>
                        <label
                          className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}
                        >
                          {t('admin.modal.fields.academicYear')}
                        </label>
                        <input
                          type="number"
                          min="1"
                    ...
                    <div className="mt-4">
                      <label
                        className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}
                      >
                        {t('admin.modal.fields.major')}
                      </label>
                      <input
                        type="text"
                        value={userForm.major}
                        onChange={(e) =>
                          setUserForm((prev) => ({ ...prev, major: e.target.value }))
                        }
                ...
                <div className="flex space-x-3">
                  ...
                  >
                    {selectedUser
                      ? t('admin.modal.buttons.update')
                      : t('admin.modal.buttons.create')}
                  </button>
                </div>
              </div>
            </div>
