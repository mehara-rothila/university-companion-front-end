...
const STATUS_CONFIG: Record<string, { color: string; bgColor: string; icon: any }> = {
    PENDING: { color: 'text-yellow-600 dark:text-yellow-400', bgColor: 'bg-yellow-100 dark:bg-yellow-900/30', icon: Clock },
    REVIEWING: { color: 'text-blue-600 dark:text-blue-400', bgColor: 'bg-blue-100 dark:bg-blue-900/30', icon: Eye },
    RESOLVED: { color: 'text-green-600 dark:text-green-400', bgColor: 'bg-green-100 dark:bg-green-900/30', icon: Check },
    DISMISSED: { color: 'text-gray-600 dark:text-gray-400', bgColor: 'bg-gray-100 dark:bg-gray-700', icon: X },
};
...
export default function AdminReportsPage() {
    ...
    const renderUserAvatar = (name: string, imageUrl?: string, size: string = 'w-10 h-10') => {
        ...
        return (
            <div className={`${size} rounded-full bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center text-white font-medium text-sm`}>
                {getInitials(name || 'U')}
            </div>
        );
    };
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10 pt-24">
                        ...
                        <div className={`flex flex-col md:flex-row md:items-center justify-between p-6 rounded-xl ${isDarkMode ? 'bg-gray-900/80' : 'bg-white/80'} backdrop-blur-sm shadow-lg gap-4`}>
                            ...
                            <div className="flex items-center space-x-4">
                                <div className={`flex rounded-lg p-1 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'} flex-wrap gap-1`}>
                                    {(['PENDING', 'REVIEWING', 'RESOLVED', 'DISMISSED', 'ALL'] as ReportStatus[]).map((status) => (
                                        <button
                                            key={status}
                                            ...
                                            className={`px-3 py-2 rounded-md text-sm font-medium transition-all ${
                                                filter === status
                                                    ...
                                                    : status === 'REVIEWING' ? 'bg-blue-600 text-white shadow-sm'
                                                    : status === 'RESOLVED' ? 'bg-green-600 text-white shadow-sm'
                                                    : status === 'DISMISSED' ? 'bg-gray-600 text-white shadow-sm'
                                ...
                                <div className={`px-4 py-2 rounded-lg ${isDarkMode ? 'bg-red-900/30 border border-red-700/30' : 'bg-red-100 border border-red-200'}`}>
                    ...
                    <div className={`${isDarkMode ? 'bg-gray-800/90 border-gray-700' : 'bg-white/90 border-gray-100'} rounded-2xl shadow-lg border backdrop-blur-sm p-6`}>
                        {loading ? (
                            ...
                            <div className="text-center py-12">
                                <Shield className={`w-16 h-16 mx-auto mb-4 ${isDarkMode ? 'text-gray-500/50' : 'text-gray-400/50'}`} />
                                <p className={`text-xl ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
                                    {filter === 'PENDING' ? 'No pending reports' :
                                     filter === 'REVIEWING' ? 'No reports under review' :
                                     filter === 'RESOLVED' ? 'No resolved reports' :
                                     filter === 'DISMISSED' ? 'No dismissed reports' :
                                     'No reports found'}
                                </p>
                                <p className={`${isDarkMode ? 'text-gray-500' : 'text-gray-500'} mt-2`}>
                                    {filter === 'PENDING' ? 'All reports have been reviewed!' : 'No reports match this filter.'}
                                </p>
                            </div>
                        ) : (
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-4 mb-3">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            {renderUserAvatar(report.reporterFullName, report.reporterImage, 'w-8 h-8')}
                                                            <div>
                                                                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Reporter</p>
                                                        ...
                                                        {/* Reported User */}
                                                        <div className="flex items-center gap-2">
                                                            {renderUserAvatar(report.reportedFullName, report.reportedUserImage, 'w-8 h-8')}
                                                            <div>
                                                                <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Reported</p>
                                                                <p className={`text-sm font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
                                                    ...
                                                    <div className="flex flex-wrap items-center gap-2 mb-2">
                                                        <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${isDarkMode ? 'bg-red-900/30 text-red-300' : 'bg-red-100 text-red-800'}`}>
                                                            {REPORT_REASON_LABELS[report.reason] || report.reason}
                                                        </span>
                                                        <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${STATUS_CONFIG[report.status]?.bgColor} ${STATUS_CONFIG[report.status]?.color}`}>
                                                            <StatusIcon className="w-3 h-3 inline mr-1" />
                                                            {report.status}
                                                        </span>
                                                        {report.conversationId && (
                                                            <span className={`px-2.5 py-1 text-xs font-medium rounded-full ${isDarkMode ? 'bg-purple-900/30 text-purple-300' : 'bg-purple-100 text-purple-800'}`}>
                ...
                {showDetailModal && selectedReport && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                        <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto`}>
                            ...
                            {/* Users Info */}
                            <div className={`p-4 rounded-xl mb-4 ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-100'}`}>
                                <div className="flex items-center justify-between">
                                    {/* Reporter */}
                                    <div className="flex items-center gap-3">
                                        {renderUserAvatar(selectedReport.reporterFullName, selectedReport.reporterImage, 'w-12 h-12')}
                                        <div>
                                            <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Reporter</p>
                                    ...
                                    <div className="flex flex-col items-center px-4">
                                        ...
                                        <span className={`text-xs mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>reported</span>
                                    </div>

                                    {/* Reported User */}
                                    <div className="flex items-center gap-3">
                                        <div className="text-right">
                                            <p className={`text-xs ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Reported User</p>
                                            <p className={`font-medium ${isDarkMode ? 'text-gray-200' : 'text-gray-800'}`}>
                                        ...
                                        {renderUserAvatar(selectedReport.reportedFullName, selectedReport.reportedUserImage, 'w-12 h-12')}
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4 mb-6">
                                <div>
                                    ...
                                    <span className={`inline-block px-3 py-1.5 text-sm font-medium rounded-lg ${isDarkMode ? 'bg-red-900/30 text-red-300' : 'bg-red-100 text-red-800'}`}>
                                ...
                                {selectedReport.description && (
                                    <div>
                                        ...
                                        <p className={`p-3 rounded-lg ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700'}`}>
                                            {selectedReport.description}
                                        </p>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <span className={`inline-block px-3 py-1.5 text-sm font-medium rounded-lg ${STATUS_CONFIG[selectedReport.status]?.bgColor} ${STATUS_CONFIG[selectedReport.status]?.color}`}>
                                ...
                                {selectedReport.conversationId && (
                                    <div>
                                        ...
                                        <span className={`inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-lg ${isDarkMode ? 'bg-purple-900/30 text-purple-300' : 'bg-purple-100 text-purple-800'}`}>
