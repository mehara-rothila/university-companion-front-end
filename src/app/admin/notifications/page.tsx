...
export default function AdminNotifications() {
  ...
  const loadNotifications = async () => {
    try {
      const response = await axios.get(`${API_BASE}/notifications/admin/all?page=${currentPage}&size=10`, {
  ...
  const toggleNotification = async (id: number) => {
    try {
      await axios.put(`${API_BASE}/notifications/admin/${id}/toggle`, {}, {
        headers: getAuthHeaders()
      });
      ...
      <main className={`min-h-screen ${isDarkMode ? 'bg-gradient-to-b from-gray-900 to-gray-800' : 'bg-gradient-to-b from-gray-50 to-gray-100'} transition-colors duration-300 relative overflow-hidden`}>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 relative z-10 pt-24">
          ...
          <div className={`glass-card ${isDarkMode ? 'bg-gray-800/90 border-gray-700' : 'bg-white/90 border-gray-100'} rounded-2xl shadow-lg p-6 border backdrop-blur-sm animate-fade-in`}>
            <h2 className={`text-xl font-semibold ${isDarkMode ? 'text-gray-100' : 'text-gray-900'} mb-6`}>
              All Notifications
            </h2>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className={`px-2 py-1 text-xs rounded-full ${
                            ...
                            {notification.target === 'ALL_STUDENTS' ? 'All Students' : 'Specific Users'}
                          </span>
                        </div>
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        ...
                        >
                          <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={notification.isActive ? "M15 12a3 3 0 11-6 0 3 3 0 016 0z" : "M13.875 18.825A10.05 10.05 0 0112 19c-4.478 0-8.268-2.943-9.543-7a9.97 9.97 0 011.563-3.029m5.858.908a3 3 0 114.243 4.243M9.878 9.878l4.242 4.242M9.878 9.878L3 3m6.878 6.878L21 21"} />
                          </svg>
                        </button>
                        <button
        ...
        {showCreateModal && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto`}>
              ...
              <div className="space-y-4">
                <div>
                  <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Title</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Target</label>
                  <select
                ...
                {notificationForm.target === 'SPECIFIC_USERS' && (
                  <div>
                    <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Select Users</label>
                    <div className={`max-h-32 overflow-y-auto border rounded-lg p-2 ${isDarkMode ? 'border-gray-600 bg-gray-700' : 'border-gray-300 bg-gray-50'}`}>
                      {users.map((user) => (
                        <label key={user.id} className="flex items-center p-2 hover:bg-gray-100 dark:hover:bg-gray-600 rounded">
                          <input
                            type="checkbox"
                ...
                <div>
                  <label className={`block text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'} mb-2`}>Expires At (Optional)</label>
                  <input
                    type="datetime-local"
