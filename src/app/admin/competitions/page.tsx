...
export default function AdminCompetitionsPage() {
  ...
  const viewEnrollments = async (competition: Competition) => {
    ...
    try {
      const data = await competitionService.getEnrollmentsAsAdmin(competition.id);
      setEnrollments(data);
    } catch (error) {
  ...
  const exportEnrollments = async (competitionId: number) => {
    try {
      await competitionService.exportEnrollmentsAsAdmin(competitionId);
    } catch (error) {
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  {enrollments.length > 0 && (
                    <button
                      onClick={() => exportEnrollments(selectedCompetition.id)}
                  ...
                  <button
                    ...
                    className={`${isDarkMode ? 'text-gray-400 hover:text-gray-200' : 'text-gray-600 hover:text-gray-800'} transition-colors`}
                  >
                    <X className="w-6 h-6" />
                  </button>
                </div>
                  ...
                  <div className="space-y-3">
                    ...
                    {enrollments.map((enrollment, index) => {
                      ...
                      return (
                        <div
                          key={enrollment.id}
                          className={`rounded-xl border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'}`}
                        >
                          <div
