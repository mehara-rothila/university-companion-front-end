...
export default function TokenUsageDisplay({
  ...
  if (!user) {
    return (
      <div
        className={`px-4 py-2 rounded-lg text-xs ${isDarkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-200 text-gray-600'}`}
      >
  ...
  if (compact) {
    ...
    return (
      <div
        className={`relative px-5 py-3 rounded-xl border-2 backdrop-blur-md shadow-lg transition-all overflow-hidden ${
          isExceeded
      ...
      >
        ...
        <div className="relative z-10 flex items-center justify-between gap-4">
          ...
          <div className="flex-1 flex items-center gap-3">
            ...
            {/* Percentage Badge */}
            <div
              className={`px-3 py-1 rounded-lg font-bold text-sm shadow-md ${
                isExceeded
      ...
      {showDetails && (
        <div className="grid grid-cols-3 gap-3">
          ...
          >
            ...
            <p
              className={`text-lg font-bold mt-1 ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}
            >
      ...
      {isExceeded && (
        <div className="mt-3 px-3 py-2 rounded-lg bg-red-100 dark:bg-red-900/30 border border-red-200 dark:border-red-700">
          <p className="text-xs text-red-700 dark:text-red-300">
            ❌ You've reached your daily token limit (500,000 tokens). Your limit will reset at
            midnight.
          </p>
        </div>
      )}
