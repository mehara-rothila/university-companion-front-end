...
export default function LostFoundInbox({ onSelectConversation, onClose }: LostFoundInboxProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div
        className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-2xl shadow-xl w-full max-w-md h-[80vh] flex flex-col`}
      >
        {/* Header */}
        <div
          ...
          {loading ? (
            ...
            conversations.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center p-4">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className={`h-16 w-16 ${isDarkMode ? 'text-gray-600' : 'text-gray-400'} mb-4`}
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                    ...
                    >
                      ...
                      <div className="flex-shrink-0">
                        {otherUser.profileImage ? (
                          ...
                          <div
                            className={`w-12 h-12 rounded-full flex items-center justify-center ${isDarkMode ? 'bg-purple-900/30' : 'bg-purple-100'}`}
                          >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between mb-1">
                          <h3
                            className={`font-semibold truncate ${isDarkMode ? 'text-gray-100' : 'text-gray-900'}`}
                          >
          ...
          pendingRequests.length === 0 ? (
            ...
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {pendingRequests.map((request) => (
                <div key={request.id} className={`p-4 ${isDarkMode ? 'bg-gray-800' : 'bg-white'}`}>
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      >
                        {t('lostFoundPage.messages.wantsToContactAbout')}{' '}
                        <span className="font-medium">{request.item.title}</span>
                      </p>
                      {request.initialMessage && (
                        <div
                          className={`p-2 rounded-lg mb-3 ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-100'}`}
                        >
                          <p
